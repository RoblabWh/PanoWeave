...
int main(int argc, char **argv)
{
    ...
    cv::createTrackbar("Resolution X (px)", "Output", nullptr, 4000, [](int pos, void *stitcher) -> void
                       { static_cast<PanoWeave::PanoWeave *>(stitcher)->width(pos > 0 ? pos : 1); }, stitcher.get());
    cv::createTrackbar("Resolution Y (px)", "Output", nullptr, 2000, [](int pos, void *stitcher) -> void
                       { static_cast<PanoWeave::PanoWeave *>(stitcher)->height(pos > 0 ? pos : 1); }, stitcher.get());
    cv::createTrackbar("FOV X (deg)", "Output", nullptr, 360, [](int pos, void *stitcher) -> void
                       { static_cast<PanoWeave::PanoWeave *>(stitcher)->fovX(pos * M_PI / 180); }, stitcher.get());
    cv::createTrackbar("FOV Y (deg)", "Output", nullptr, 180, [](int pos, void *stitcher) -> void
                       { static_cast<PanoWeave::PanoWeave *>(stitcher)->fovY(pos * M_PI / 180); }, stitcher.get());
    cv::createTrackbar("Depth (cm)", "Output", nullptr, 1000, [](int pos, void *stitcher) -> void
                       { static_cast<PanoWeave::PanoWeave *>(stitcher)->setDepth(pos > 0 ? pos / 100. : 0.1); }, stitcher.get());
    cv::createTrackbar("Vignette Threshold (%)", "Output", nullptr, 100, [](int pos, void *stitcher) -> void
                       { static_cast<PanoWeave::PanoWeave *>(stitcher)->vignetteThreshold(pos / 100.); }, stitcher.get());
    cv::createTrackbar("Rotation X (deg)", "Output", nullptr, 360, [](int pos, void *stitcher) -> void
                       { auto rvec = static_cast<PanoWeave::PanoWeave *>(stitcher)->transform().rvec();
                         rvec[0] = pos / 180. * M_PI;
                         static_cast<PanoWeave::PanoWeave *>(stitcher)->transform(rvec); }, stitcher.get());
